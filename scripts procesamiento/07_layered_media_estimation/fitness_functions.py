    ...
    for rx in range(1, nrx + 1):
        ...
        if output.attrs['Name'] in args['outputs']:
            ...
            if args['type'] == 'min':
                ...
                raise Exception('type must be either min, max, or absmax')
            outputsused = True

    # Check in case no outputs where found
    if not outputsused:
        raise Exception('No outputs matching {} were found'.format(args['outputs']))
...
def xcorr(filename, args):
    ...
    if not os.path.isfile(refrespfile):
        raise Exception('Cannot load reference response from {}'.format(refrespfile))
    with open(refrespfile, 'r') as f:
        refdata = np.loadtxt(f)
    refdata = refdata[np.unique(refdata[:, 0], return_index=True)[1]]
    reftime = refdata[:, 0]
    refresp = refdata[:, 1]
    ...
    for rx in range(1, nrx + 1):
        output = f['/rxs/rx' + str(rx) + '/']
        print(f"Name {output.attrs.keys()}")
        if output.attrs['Name'] in args['outputs']:
            outputname = list(output.keys())[0]
            print(outputname)
            ...
            print(f.attrs.keys())
            if outputname == 'Ex':
                modelresp *= -f.attrs['dx_dy_dz'][0]
            elif outputname == 'Ey':
                modelresp *= -f.attrs['dx_dy_dz'][1]
            elif outputname == 'Ez':
                modelresp *= -f.attrs['dx_dy_dz'][2]
            outputsused = True
    ...
    f = interpolate.interp1d(reftime, refresp, kind = 'linear', fill_value='extrapolate')
    print(modeltime)
    refresp = f(modeltime)
    reftime = modeltime
    ...
    modelresp = (modelresp - np.mean(modelresp)) / (np.std(modelresp) *len(modelresp))
    ...
    ax.plot(reftime, refresp,'r', lw=2, label='refresp')
    ax.plot(modeltime, modelresp,'b', lw=2, label='modelresp')
    ax.grid()
    plt.show()
...
def min_sum_diffs(filename, args):
    ...
    refrespfile = os.path.abspath(args['refresp'])
    if not os.path.isfile(refrespfile):
        raise Exception('Cannot load reference response at {}'.format(refrespfile))
...
args = {
    'refresp': r"C:\Users\leone\OneDrive - Universidad de los Andes\Universidad\Proyecto Desminado\Bow-Tie Antenna\GSSI Ey.txt",
    'outputs': 'rxbowtie'
}

print(f"Correlation: {xcorr('antenna_gssi_base.out', args)}")
